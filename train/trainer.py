...
class Trainer:
    ...
    def loss_inr(self, data, drec):
        ...
        loss_firstframe = 50 * self.LossL1(data['motion_imgs'][:, :, 0], drec['imgs'].view(bs, height, width)[:, :, 0])
        loss_lastframe = 50 * self.LossL1(data['motion_imgs'][:, :, -1], drec['imgs'].view(bs, height, width)[:, :, -1])
